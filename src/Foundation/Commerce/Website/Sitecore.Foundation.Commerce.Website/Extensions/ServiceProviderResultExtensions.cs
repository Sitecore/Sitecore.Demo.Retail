...
namespace Sitecore.Foundation.Commerce.Website.Extensions
{
    ...
    {
        public static void WriteToSitecoreLog(this ServiceProviderResult result)
        {
            if (result.Success)
                return;
            var systemMessages = result.SystemMessages.ToList();
            foreach (var message in systemMessages)
            {
                Sitecore.Diagnostics.Log.Error(message.Message, result);
            }
        }
    }
}