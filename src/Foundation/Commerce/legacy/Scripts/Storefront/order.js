...
function addOrderItemsToCart() {
    ...
    ClearGlobalMessages();
    AJAXPost(StorefrontUri('api/storefront/orders/reorder'), JSON.stringify(data), function (data, success, sender) {
...
function cancelOrderItems() {
    ...
    ClearGlobalMessages();
    AJAXPost(StorefrontUri('api/storefront/orders/cancelorder'), JSON.stringify(data), function (data, success, sender) {
        if (success && data.Success) {
            $('.item-to-selected').removeAttr('checked');
            $('#selectAllOrderItems').removeAttr('checked');
            $("#addToCartSuccess").show().fadeOut(4000);
