...
namespace Sitecore.Demo.Retail.Feature.Catalog.Website.Extensions
{
    public static class UrlHelperExtensions
    {
        public static string AddToFacets(this UrlHelper helper, string facetName, string facetValue)
        {
            ...
            var facetQuery = currentUrl.QueryList[Constants.QueryString.Facets];
            var facetQueryString = GetFacetQueryString(facetQuery, facetName, facetValue);

            if (!string.IsNullOrEmpty(facetQueryString))
            {
                currentUrl.QueryList.AddOrSet(Constants.QueryString.Facets, HttpUtility.UrlDecode(facetQueryString).Remove(0, 1));
            }
            else
            {
                currentUrl.QueryList.Remove(Constants.QueryString.Facets);
            }

            currentUrl.QueryList.Remove(Constants.QueryString.Paging);
        ...
        private static string GetFacetQueryString(string facetQuery, string facetName, string facetValue)
        {
            ...
            if (facetCollection.Contains(facetName))
            {
                ...
                {
                    var facetValues = facetQueryValues.Split(Constants.FacetsSeparator).Where(p => !string.Equals(p, facetValue, StringComparison.OrdinalIgnoreCase)).ToList();
                    if (facetValues.Any())
                    {
                        facetCollection.Set(facetName, string.Join(Constants.FacetsSeparator.ToString(), facetValues));
                    }
                    else
                    {
                ...
                {
                    facetCollection.Set(facetName, facetQueryValues + Constants.FacetsSeparator + facetValue);
                }
            }
            else
            {
